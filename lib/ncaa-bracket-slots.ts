import type { RoundId } from "@/lib/bracket-types";

/**
 * Map NCAA championship `bracketPositionId` (henrygd scoreboard `bracketId`) to our
 * `round` + `slotId` (see lib/bracket-tree.ts). Mirrors scripts/lib/ncaa-bracket-slots.mjs.
 */
export function bracketPositionToOurSlot(bracketPositionId: number | string): {
  round: RoundId;
  slotId: string;
} | null {
