...
**Official results (NCAA API):** Run `npm run ncaa:sync` to pull finished games from [henrygd NCAA API](https://ncaa-api.henrygd.me) into `data/tournament-results.json`. The site highlights the real winner on each matchup and shows whether the viewed bracket matches. Optional env `NCAA_API_BASE` if you self-host the proxy; optional `TOURNAMENT_YEAR` (otherwise `--year` or current UTC year). If the sync logs `UNRESOLVED_SEO`, add a line to `data/ncaa-seo-overrides.json` (`ncaa_seoname` → `teams.json` id). First Four (bracket positions 101–104) is not mapped automatically; add those rows by hand under `actuals.first_four` if you need them.

**Automated sync (GitHub Actions):** [`.github/workflows/sync-ncaa-results.yml`](.github/workflows/sync-ncaa-results.yml) runs every 15 minutes on **tournament days only** (Pacific: Mar 21–22, 26–27, 28–29; Apr 4, 6) between **9:00 AM and 9:59 PM** local time. Each run uses **one** brackets API request. It commits and pushes **only if** `data/tournament-results.json` changes, so Vercel only rebuilds when there is new data (easy on Hobby build hours). Enable **Workflow permissions → Read and write** for Actions in the repo settings so the default `GITHUB_TOKEN` can push. Update `TOURNAMENT_YEAR` in the workflow when the season changes. Use **Actions → Sync NCAA tournament results → Run workflow** with **force** to run outside that window (e.g. testing).
